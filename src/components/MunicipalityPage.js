...
class MunicipalityNeeds extends Component {
  ...
  componentDidMount () {
    ...
    d3
      .csv(
        'https://docs.google.com/spreadsheets/d/e/2PACX-1vSQebIhEjhFR3LewIiByv3yfqc2YY0GH-cO5mXjhfYDfJY5Z7vVGvtsVSKN-CjtZhNxe0gOzHN0_bDN/pub?gid=534002250&single=true&output=csv'
      )
      .then(data => {
        var desc = data.reduce((obj, row) => {
          obj[row['Indicator Variable']] = row['Indicator_Description']
            .replace(
              'Number/Proportion of individuals with SSS / GSIS (up to barangay level)',
              'Proportion of individuals without SSS / GSIS (up to barangay level)'
            )
            .replace(
              'Number/Proportion of households which received PhilHealth, Pantawid, and other Cash Transfers from government/LGU/NGO (up to barangay level)',
              'Proportion of households which did not receive PhilHealth, Pantawid, and other Cash Transfers from government/LGU/NGO (up to barangay level)'
            )
          return obj
        }, {})
        this.setState({
    ...
    d3
      .csv(
        'https://docs.google.com/spreadsheets/d/e/2PACX-1vSQebIhEjhFR3LewIiByv3yfqc2YY0GH-cO5mXjhfYDfJY5Z7vVGvtsVSKN-CjtZhNxe0gOzHN0_bDN/pub?gid=1343037281&single=true&output=csv'
      )
      .then(data => {
        var expl = data.reduce((obj, row) => {
          obj[row['Need Variable']] = {
            text_before: row['Text Before'],
            text_after: row['Text After'],
            type: row['Type']
          }
          return obj
        }, {})
        ...
        <ul className='mun-sidebar-list'>
          {needKeys.map((need, i) => (
            <MunicipalityNeedItem
              key={need}
              score={
                municipalityScores[this.state.munCode][needs[need]['prop-col']]
              }
              clickMethod={() => this.toggleAccordion(need)}
              className={
                this.state.openNeed === need
                  ? 'mun-sidebar-item active'
                  : 'mun-sidebar-item'
              }
              need={need}
...
class MunicipalityPage extends Component {
  ...
  componentDidMount () {
    d3
      .csv(
        'https://docs.google.com/spreadsheets/d/e/2PACX-1vSQebIhEjhFR3LewIiByv3yfqc2YY0GH-cO5mXjhfYDfJY5Z7vVGvtsVSKN-CjtZhNxe0gOzHN0_bDN/pub?gid=0&single=true&output=csv'
      )
      .then(data => {
        Object.keys(data || '')
          .filter(
            index => this.props.match.params.munCode === data[index].MunCode
          )
          .map(index => this.setState({ munData: data[index] }))
      })

    d3
      .csv(
        'https://docs.google.com/spreadsheets/d/e/2PACX-1vSQebIhEjhFR3LewIiByv3yfqc2YY0GH-cO5mXjhfYDfJY5Z7vVGvtsVSKN-CjtZhNxe0gOzHN0_bDN/pub?gid=660297486&single=true&output=csv'
      )
      .then(ranks => {
        var ranksIndex = 0
        var totalIndex = 0
        ...
        Object.keys(ranks || '')
          .filter(
            index => this.props.match.params.munCode === ranks[index].Mun_Code
          )
          .map(index => (ranksIndex = index))
        Object.keys(ranks || '')
          .filter(index => ranks[index].Mun_Code === 'Total')
          .map(index => (totalIndex = index))
        ...
        <Fragment>
          ...
          >
            ...
            <div className='mun-sidebar-label-div'>
              <div className='mun-sidebar-label' />
            </div>
